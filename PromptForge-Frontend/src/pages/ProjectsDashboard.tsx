...
import {
  Plus, LogOut, Search, Folder, Loader2, MoreVertical, Trash, Download, Edit, CreditCard, Shield
} from "lucide-react";
...
import { api, removeAuthToken, removeUserInfo, getUserInfo, isAuthenticated, clearAppSessionState } from "@/lib/api";
...
export function ProjectsDashboard() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const isAuthError =
        /unauthorized|authentication required|missing or invalid authorization header/i.test(message);
      if (isAuthError) {
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <div className="flex flex-col space-y-1 p-2">
                ...
                <p className="text-xs leading-none text-muted-foreground">{userInfo?.username ?? ""}</p>
              </div>
      ...
      <main className="container max-w-screen-2xl py-8 px-4 sm:px-8">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage and create your AI-powered projects</p>
          </div>
                ...
                <CardContent className="p-4 flex flex-col gap-2">
                  ...
                  {project.role && (
                    <div className="flex flex-wrap gap-2">
                      ...
                      {project.role !== "OWNER" && (
                        <span className="text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border bg-blue-500/10 text-blue-500 border-blue-500/20">
                          Shared with you
                        </span>
                      )}
                    </div>
