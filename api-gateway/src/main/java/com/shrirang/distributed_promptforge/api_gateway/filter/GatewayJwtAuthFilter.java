package com.shrirang.distributed_promptforge.api_gateway.filter;

import com.shrirang.distributed_promptforge.api_gateway.config.SecurityProperties;
import com.mayur.distributed_promptforge.api_gateway.error.ApiError;
import com.shrirang.distributed_promptforge.api_gateway.service.JwtGatewayService;
...
public class GatewayJwtAuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 2. Check Redis blacklist (reactive, non-blocking)
        // defaultIfEmpty(false) handles Redis returning empty Mono (e.g. key not found or Redis hiccup)
        // onErrorReturn(false) ensures Redis downtime does not block all requests — fail open on blacklist
        return reactiveRedisTemplate.hasKey(BLACKLIST_PREFIX + token)
                .defaultIfEmpty(false)
                .onErrorReturn(false)
                .flatMap(blacklisted -> {
                    if (Boolean.TRUE.equals(blacklisted)) {
                        log.warn("Blacklisted JWT rejected at Gateway for path: {}", path);
                        return sendErrorResponse(exchange, HttpStatus.UNAUTHORIZED, "Token has been invalidated. Please log in again.");
                    }
                    return chain.filter(exchange);
                });
    }
